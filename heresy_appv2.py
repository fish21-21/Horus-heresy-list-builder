...
if st.session_state.army is not None:
    ...
    # Filter available weapons for this unit
    available_weapons = []
    for w, props in weapons.items():
        if props["allowed_units"] == "all" or unit_option.name in props["allowed_units"]:
            available_weapons.append(w)

    # Multi-select for weapons
    weapon_choice = st.multiselect("Select Weapons", available_weapons)

    # enforce max per weapon
    weapon_counts = {}
    for w in weapon_choice:
        count = weapon_choice.count(w)
        max_allowed = weapons[w]["max_per_unit"]
        if count > max_allowed:
            st.warning(f"{w} can only be taken {max_allowed} per unit. Reducing count.")
            count = max_allowed
        weapon_counts[w] = count

    # Add / undo / clear functionality
    col1, col2, col3 = st.columns([1, 1, 1])
    with col1:
        if st.button("Add Unit"):
            total_weapon_points = sum(weapons[w]["points"] * c for w, c in weapon_counts.items())
            total_unit_points = unit_option.points + total_weapon_points
            weapons_str = ", ".join([f"{w} x{c}" for w, c in weapon_counts.items()]) if weapon_counts else "None"
            unit_with_weapon = Unit(f"{unit_option.name} ({weapons_str})", total_unit_points, unit_option.category)
            ok, msg = army.add_unit(unit_with_weapon)
            if ok:
                st.success(msg)
            else:
                st.error(msg)
